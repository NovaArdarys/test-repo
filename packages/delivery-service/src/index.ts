...
const app = new Hono<{ Variables: Variables; }>()
  ...
  .use(
    '/api/*',
    cors({
      origin: ['localhost', 'https://sip-mbg.bappenas.go.id', 'http://localhost:5173', 'http://128.199.77.145:3001', 'https://dev-mbg.midigi.id'],
  ...
  .get('/swagger/mobile', swaggerUI({ url: process.env.NODE_ENV === 'DEVELOPMENT' ? '/api/mobile/openapi.json' : '/delivery/api/mobile/openapi.json' }))
  .get('/swagger', swaggerUI({ url: process.env.NODE_ENV === 'DEVELOPMENT' ? '/api/openapi.json' : '/delivery/api/openapi.json' }))
